...
let EditComponent = class EditComponent {
    ...
    save() {
        ...
        console.log(this.usr.id + "user id in  id " + this.userEntries.userId);
        if (this.entriesType === "1") {
            this.entriesService.update(this.usersEntriesEdit).subscribe(data => {
                console.log('Success: ');
                this.route.navigate(['/viewEntries']);
            }, error => {
                console.log('Error: ', error);
        ...
        if (this.entriesType === "2") {
            this.entriesService.updateEntriesForFiles(this.currentFileUpload, this.usr.id, this.usersEntriesEdit.id).subscribe(data => {
                console.log('Success: ');
                this.route.navigate(['/viewEntries']);
            }, error => {
                console.log('Error: ', error);
...
let EntriesService = class EntriesService {
    ...
    updateEntriesForFiles(file, useruserId, id) {
    ...
    saveEntriesForFiles(file, useruserId) {
        console.log("useruserId " + useruserId);
        const formdata = new FormData();
        formdata.append('file', file);
        formdata.append('userId', useruserId);
...
let RegisterGuestComponent = class RegisterGuestComponent {
    constructor(route, loginService, alertService) {
        this.route = route;
        this.loginService = loginService;
...
let ViewEntriesComponent = class ViewEntriesComponent {
    ...
    loadData() {
        ...
        if (this.usr.userRole === 'admin')
            ...
            this.isAction = false;
        this.entriesService.getEntriesList(this.usr).subscribe(data => {
    ...
    downloadfile(valId) {
        ...
        this.entriesService.downloadfile(valId).subscribe((response) => {
